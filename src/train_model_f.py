"""
Model F — structure-conditioned cGAN + MediaPipe landmark-consistency loss.
          Fusion of Model C (structure conditioning + aligned L1) and Model B's
          frozen landmark regressor. Landmark target is computed on-the-fly as
          R(real_aligned_target) so it needs NO precomputed cache and supervises
          ALL samples (not just the ~27% MediaPipe-detected). 128x128, repo-local.
...
def train():
    C.setup_speed()
    paths = C.make_dirs(C.DRIVE_BASE_F)
    ...
    # ---- Model B's frozen landmark regressor (the "F" ingredient) ----
    ...
    lam_lm_var = tf.Variable(0.0, trainable=False, dtype=tf.float32)  # warms 0 -> LAMBDA_LM_END

    _lm_warm = int(os.environ.get("ARASL_LM_WARMUP", C.WARMUP_LM_EP))

    def lam_lm(ep):
        if ep < _lm_warm:
            return 0.0
        t = (ep - _lm_warm) / max(C.WARMUP_EP, 1)
        return float(min(C.LAMBDA_LM_START + t * (C.LAMBDA_LM_END - C.LAMBDA_LM_START), C.LAMBDA_LM_END))
    ...
    start_ep = 0
    hist = {k: [] for k in ["d", "g", "g_adv", "g_l1", "g_lm"]}
    ...
    def train_step(real, y, cond):
        ...
        with tf.GradientTape() as t:
            ...
            # landmark-consistency: frozen regressor R applied to fake vs aligned real
            # target (stop-grad). Backprops through R (frozen) into G -> pushes G to
            # place hand keypoints like the real target. Dense (all samples).
            lm_fake = R(fake, training=False)
            lm_real = tf.stop_gradient(R(tf.cast(real, fake.dtype), training=False))
            g_lm = tf.reduce_mean(tf.square(lm_fake - lm_real))
            g_loss = g_adv + g_l1 + lam_lm_var * g_lm
            g_loss_s = scaled(g_opt, g_loss)
        apply_grads(g_opt, t.gradient(g_loss_s, G.trainable_variables), G.trainable_variables)
        return d_loss, g_loss, g_adv, g_l1, g_lm
    ...
    for epoch in range(start_ep, C.EPOCHS):
        ...
        lam_lm_var.assign(lam_lm(epoch))   # landmark-loss warmup schedule
        dl = gl = ga = l1 = lm = k = 0.0
        for real, y, cond in ds:
            d, g, a, p, ml = train_step(real, y, cond)
            dl += float(d); gl += float(g); ga += float(a); l1 += float(p); lm += float(ml); k += 1
        hist["d"].append(dl / k); hist["g"].append(gl / k)
        hist["g_adv"].append(ga / k); hist["g_l1"].append(l1 / k); hist["g_lm"].append(lm / k)
        _el = time.time() - _t0
        hist["epoch_seconds"].append(round(_el, 1))
        print(f"Ep{epoch+1}/{C.EPOCHS}  D={dl/k:.4f}  G={gl/k:.4f}  "
              f"(adv={ga/k:.4f}  L1={l1/k:.4f}  LM={lm/k:.4f} lam={lam_lm(epoch):.2f})  {_el:.1f}s", flush=True)
        ckpt_mgr.save()
        with open(paths["progress"], "w") as f:
            json.dump({"last_epoch": epoch + 1, **hist}, f)
        G.save_weights(os.path.join(paths["ckpt"], "generator_F.weights.h5"))
        D.save_weights(os.path.join(paths["ckpt"], "discriminator_F.weights.h5"))
